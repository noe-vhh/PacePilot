...
class UserActiveState extends State<UserActive> {
  ...
  Future<void> fetchAndSetUserActivitySummary() async {
    ...
    final result = await userActiveService.fetchUserActiveSummary(
        widget.accessToken, selectedPeriod);

    if (result['success']) {
      // Extract total active time from the result and update state
      final totalActiveSeconds =
          result['totalActiveTime']?.toDouble() ?? 0;
    ...
    if (activeTime != null) {
      ...
      return Container(
        ...
        child: buildProfileInfo(
            'Active Time ($selectedPeriod)', formattedTime),
      );
    } else {
      ...
      return Container(
        alignment: Alignment.center,
        child: buildProfileInfo(
            'Active Time', 'No activities recorded for the selected period.'),
      );
    }
  }
